...
Fisheries modelling demands extensive technical expertise and time-consuming computational workflows. The complexity of modelling can be one impediment to the delivery of timely and well-informed advice to fishery managers. For example, stock assessment reports often cite time constraints as a barrier to conducting comprehensive analyses, with professionals struggling to complete thorough sensitivity analyses or explore multiple scenarios (Punt et al. 2025). Fisheries science ecology more generally relies on a broad variety of quantitative methods, which can be difficult to learn. Quantitative environmental scientists rarely have formal training in programming, so creating and maintaining computer code is a challenge \[Leigha find ref, see below for ideas, \]. Creating code that other people can easily understand and use is an even greater challenge. Thus, there are a limited number of trained modelers that also understand the complexities of fishereries sciences, creating a bottleneck for fisheries stock assessments and compounding issues with constraints on data availability \[leigha ref here\].

Potential refs: I think there was a paper on graduate shcool training in R in the USA? (Jenkins et al. 2023; Popovic et al. 2024) Jenkins, Gareth B, Andrew P Beckerman, Céline Bellard, Ana Benı́tez-López, Aaron M Ellison, Christopher G Foote, Andrew L Hufton, et al. 2023. “Reproducibility in Ecology and Evolution: Minimum Standards for Data and Code.” Ecology and Evolution 13 (5): e9961. Popovic, Gordana, Tanya Jane Mason, Szymon Marian Drobniak, Tiago André Marques, Joanne Potts, Rocío Joo, Res Altwegg, et al. 2024. “Four Principles for Improved Statistical Ecology.” Methods in Ecology and Evolution 15 (2): 266–81. https://doi.org/https://doi.org/10.1111/2041-210X.14270.

@scheepens2024

Cooper, N., Clark, A.T., Lecomte, N., Qiao, H. and Ellison, A.M., 2024. Harnessing large language models for coding, teaching and inclusion to empower research in ecology and evolution. Methods in Ecology and Evolution, 15(10), pp.1757-1763.

Punt, A.E., Dichmont, C.M., Dowling, N.A., Deng, R.A., Pascoe, S., van Putten, I., Bessell-Browne, P., Burch, P., Burnell, O., Chick, R.C. and de Lestang, S., 2025. Identifying capacity limitations and training needs using a stock assessment game. *Fisheries Research*, *284*, p.107319.

Large language models (LLMs) and AI agents offer a transformative opportunity to automate complex fisheries modelling workflows. By separating code creation from model design and data acquisition, these technologies can potentially lower the technical barriers to entry. LLM-generated code is produced quickly, cheaply and speeds up routine tasks (Cooper et al. 2024). \[leigha fill out with examples from other fields, e.g. Scheepens?\] However, questions remain about the reliability and quality of this AI-generated code (Cooper et al. 2024) - \[Leigha elaborte on what these questions are\]. To effectively use LLM for coding, users need to be familiar enough with the programming language to detect anomalies in the AI outputs (Cooper et al. 2024).

Previous tests, primarily in the R language, have revealed significant challenges for the application of generative AI to statistics. While LLMs can generate executable code, less than 50% of generated scripts were technically accurate [@jansen2025leveraging]. Notably, code execution rates improved when AI models could self-correct [@zhu2024large]. Key insights have emerged about effective AI interaction: Contextual information is crucial for making informed statistical choices Providing reference code and statistical approaches guides AI agents One-shot prompting with comprehensive examples often yields more reliable results than multi-turn conversations [@laban2025llms]. Users must be skillfull in writing effective prompts—instructions or inputs given to the LLM, and evaluating the responses (Cooper et al. 2024). The development of detailed specification sheets—long, precise instruction sets (i.e. prompts) written in plain English—represents a promising approach to improving AI-generated analysis. These specifications can articulate analysis requirements clearly, making them accessible to both expert and novice users.

We test whether AI agents can transform specification sheets into reliable fisheries models by examining three increasingly complex tasks: (1) von Bertalanffy growth curve fitting, (2) generalized linear modeling of fish-habitat relationships, and (3) yield per recruit analysis. We conduct detailed analysis of the generated responses, in order to better understand the strengthts and limitations of the AI. Evaluations of agent-LLM pairs in other fields suggest we should be successful. However, fisheries science may present a special challenge to agents, because of the relatively limited amount of training data available to LLMs relative to topics such as game programing. We are also testing the agents in the R program, which is popular with environmental statisticians, but for which there is much more limited training code available than common languages like C.
...
Up to here, have re-structured methods. Now write them.
...
Up to here, write up YPR results. Basically Sonnet did an ok job. But often stuffed up the bit abotu order of fishing and natural mortality, or applied moretalty to the first age class despite instructions otherwise. Once again, Kimi was not as good, though we note had some high aptitude scores. It frequently failed to finish the tasks... Todo alex to add further anallysis of the qualitative results.

![](Shared/Outputs/figure-4_ypr.png)
...
Kimi was less reliable if left on its own. Basiscally would be good as an AI aid, but not trusted to work on its own. Its work needs more correction and checking. Note that could be interation with Roo Code set-up which is optimised for Sonnet.
