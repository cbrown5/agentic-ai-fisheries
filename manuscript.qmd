...
All analyses (including agent led and human led) were conducted with the R program [@rcoreteam]. Generative AI was used to assist creating code and for editorial advice on the writing (details in supplemental methods).
...
We instructed the replicate agents to complete a csv file of parameter values (with the same structure as Table S1). The agent was given an empty csv file that had only headings, such that it provided results to us in a standardized way. We then scored the responses for accuracy on each parameter and confidence interval (correct answers in Table S1). Each replicate and parameter estimate was scored a `1` if it was within 5% of our our estimated value and scored `0` otherwise.
