...
Need to standardize language. "Fisheries and ecological modeling". Agentic AI vs AI agents...

Standarize figure labels. Should say Claude Sonnet 4.0 and Kimi K2 on all.

YPR results, need to check NAs in ypr_long_scored.csv column 'is_correct'. Are NAs counted as not correct?

re-run YPR plot with 15 removed.

Add references, find refernces where we need them. Any help appreciated!
...
One author (CJB) then manually marked the reports on criteria for: Interpretation. Were statistics correctly interpreted, did the agent come to the correct conclusion? Completeness. Did the agent complete all the tasks we requested? Technical implementation: Did the agent create the figures we requested, and did the scripts it write all run without errors (for some replicates the agent wrote the scripts but did no run them)
...
![](Shared/Figure-comparison-png.png) **Figure 2** Examples of accurate and erroneous output. (a) The von Bertlanffy case-study, where all replicates produced accurate plots of the growth curve. Examples of Sonnet 4.0 producing predictions from the GLM on the appropriate (b) and inappropriate (c) y-axis scale. Examples of (d) Sonnet 4.0 producing accurate yield-per-recruit curves and (e) Kimi K2 producing inaccurate yield-per-recruit curves.

### VBF

The the Sonnet LLM with the full specification completed the von Bertlanaffy task with perfect accuracy across all 10 replicates (fig. 3). The Kimi LLM estimated most parameters with more than 80% accurate. However, we note that Kimi's output needed some cleaning before we could produce summary results. In particular, the LLM used names for the 'length at age 5' parameter that were inconsistent with the convention we proposed in the spec.

The chat logs revealed that in two replicates Kimi made simple syntax errors in the R code, resulting in a script that did not complete the task error free. The agent however failed to realize it had not completed the task fully (a confabulation) and proceeded to manually entered parameter values into the parameter table, putting those values in the wrong positions.
...
The Kimi LLM did not perform reliably or accurately on the GLM task (fig 4). It frequently failed to complete reports (no report created 30% of the time) or did not follow instructions precisely. It often created code for the likelihood ratio test, but then used a different or made-up p-value in reporting (fig. 4a). In some cases the LLM generated code for plots that executed successfully when we tested it, but did not complete the full reporting task. We found cases of fabrication in the chat logs for the Kimi agent. The Kimi agent often stated it had completed the reports, although it had not done so. The only criteria where the Kimi LLM outperformed the Sonnet LLM was on using the correct scale for the predictive plots (fig. 4a).

The Kimi LLM had high aptitude for completeness (92%) and technical implementation (96%) (fig 4b), indicating that it could perform well at these types of tasks, just not reliably.
...
The Sonnet LLM demonstrated moderate performance on the yield per recruit (YPR) analysis (fig. 5). Sonnet consistently completed the assigned tasks, generating executable code, producing plots, and populating parameter tables as specified. In contrast, the Kimi LLM was less reliable, frequently failing to complete the workflow. Kimi often produced code that contained errors or was not executed, resulting in incomplete outputs.
...
We compared the average accuracy scores against total cost for the quantitative evaluations for the VBF and YPR case-studies (fig. 6). There was a pattern of Kimi's accuracy being lower but it was also cheaper to run than Sonnet 4.0. However, within a model there was no trend of higher cost solutions tending to have lower accuracy. For example, Kimi failed two times to complete the VBF task, in both of those times it got stuck in an endless tool calling loop, wasted tokens and never found a solution (fig. 6a). Sonnet's most expensive solutions for the YPR also tended to be less accurate.

![](Shared/Outputs/figure-5-cost-accuracy.png)

## 4. Discussion

This study shows that agentic AI systems can accurately and consistently automate common fisheries modelling workflows. We also showed that performance varies across current state-of-the-art LLMs. Our evaluation of AI agents across three increasingly complex fisheries modelling tasks—von Bertalanffy growth fitting, generalized linear modelling, and yield per recruit analysis—reveals that Claude Sonnet 4.0 consistently achieved high accuracy (near 100% for simple tasks, moderate performance for complex tasks), while Kimi K2 showed variable and often unreliable performance. Our results align with evaluations of LLMs from other areas of ecology that also find high performance when prompts and agent software are calibrated to specific tasks. This includes automated literature synthesis [@scheepens2024, @gougherty2024] and ecological model generation [@Spillias2025ecosystemmodel], [Leigha add other refs here?]. A key point of difference between our study and earlier studies that looked at model and code development is that we used generic agent software, rather than software that was calibrated to a specific science application [e.g. @Spillias2025ecosystemmodel; @jansen2025leveraging]. Our results are important because they pave the way for immediate integration of LLMs and agents into workflows for ecological and fisheries modelling.

The performance patterns revealed distinct capabilities and limitations across different types of modeling tasks and LLMs. It is likely that LLM choice will remain an important decision in the near-term. Claude Sonnet 4.0 showed consistent high performance across the complexity gradient from simple parameter estimation to complex population modeling, maintaining reliable task completion and accurate statistical interpretation. In contrast, Kimi K2 exhibited significant variability, with particular weaknesses in complex analytical workflows where it frequently failed to complete tasks, made confabulations where it thought it had completed a task it had not completed or generated plausible but incorrect outputs—a form of hallucination particularly concerning in scientific applications. Evaluations of LLMs on logic and reasoning tasks often find highly variable performance across different LLMs [e.g. \@laban2025llms; \@zhu2024large](Leigha%20other%20refs%20here?). More generally, open-source models like Kimi K2 do not yet have the complexity of proprietary LLMs like Sonnet 4.0, so often perform more poorly on logic and reasoning tasks [e.g. @laban2025llms; @zhou2024large].

These findings align with broader trends in AI-assisted scientific computing while revealing domain-specific challenges for fisheries applications [@bistarlli2025; @scheepens2024; @bran2024]. Unlike general programming tasks where LLMs benefit from extensive training data, fisheries modeling represents a specialized domain with limited publicly available code and methodological examples. This scarcity likely contributes to the variable performance we observed, particularly for complex tasks requiring integration of multiple biological parameters and statistical methods. The scarcity of information was potentially reflected in some of the quirks of the model output. For example, in the GLM case-study Sonnet 4.0 was inconsistent about providing predictions on a log-transformed scale, despite our specific request to do so. Sonnet 4.0 also consistently applied natural mortality to new recruits in the yield-per-recruit model, despite our request not to do that. These minor inconsistencies in otherwise complete analyses highlight the importance of human oversight and review of code. Further, none of the agents looked for confounding among variables in the GLM case-study, despite that being part of best-practice statitistcal workflows [@zuur2009mixed]. We suggest agent software can be used by experts to speed up coding and model development, but is not a replacement for experts. Expert knowledge is required to create precise specification sheets that describe the full modelling workflow. The resulting code and results also need need to be reviewed by an expert, so that errors and oversights can be idenfied and corrections made.

Several important limitations must be considered when interpreting these results and their applicability to real-world fisheries applications. Our evaluation was limited to three modeling tasks that used simulated or controlled datasets, which may not capture the complexity and messiness of real-world fisheries data. An important next step would be to study agent performance on larger workflows, such as a full stock assessment that requires fitting multiple statistical models to estimate life-history parameters (e.g. growth, mortality, selectivity), before fitting a biomass dynamics model. For ecological applications of agents it would be useful to study complex multivariate datasets (e.g. ref) and statistical problems that require aligning multiple different databases (data wrangling). These are important frontiers for automation, for instance automated data wrangling remains elusive (ref - on automateing data wrangling with LLMs). Even though our workflows were simplified from real world problems, the high performance we found still suggests that many components of real world workflows can be partly automated, potentially saving days of modeller time. We also tested only one agent platform (Roo Code) and two LLMs, and results may not generalize to other AI systems or configurations. Specifically, Roo Code has been optimized for the Claude Sonnet series of models, whereas Kimi K2 is a newer model. It may be possible to obtain better results for Kimi K2 with different agent software. The Kimi K2 LLM may also still be useful to experts who are able to take the code as a starting point to more quickly complete a modelling task. Additionally, LLMs and their integration into agents is evolving rapidly, meaning our performance results may quickly become outdated. These limitations do not undermine our core findings because we have established an approach for performance evaluation of agents under controlled conditions; our approach allows systematic comparison between models and provides a foundation for future evaluations with the next generation of LLMs or with other modelling tasks.

Our study provides practical lessons for the integration of agents into modelling workflows. First, we have designed a workflow for automating modelling tasks and proven that it is accurate and reliable. The primary step in our workflow is to instruct agents with detailed specification sheets that cover analysis aims, methodology, software preferences, meta-data and instructions for outputs. Our approach extends previous LLM evaluations in the ecological sciences, which have tested quality of responses to one-off prompts [@scheepens2024] or simple agents that iterate to correct syntax errors [@jansen2025leveraging]. The next challenge will be to integrate agents into real-world modelling workflows. There are several important considerations that need to be made for integration into real workflows. First, the the creation of specification sheets takes careful thought from modelling experts. Generative AI could be used to assist in this creation, but expert oversight is essential because any logical errors in the specification sheets will likely flow through to the agent generating misleading results. Second, trust in modelling approaches that lean on AI is critical for advancing scientific knowledge and management applications of modelling. Scientists using generative AI are required to be transparent about its use and take full responsibility for mistakes [@cooper2024] and we do not recommend the use of code generated by AI without review of the code by experts (colloquially called 'vibe-coding'). Agents are therefore a tool for speeding up development of models by experts, rather than a tool for modelling novices to generate management advice. LLMs can be useful aids for novices to learn quantitative methods, but they cannot replace the need for humans to learn math, statistical reasoning and computer coding [@campbell2024]. Further, it is critical that humans retain understanding of their research [@messeri2024artificial], without that knowledge scientists will be unable to provide informed advice to stakeholders. Retaining expert knowledge is especially critical in fisheries science, where methods need to be transparent to the public and scientists need to be able to explain stock models to stakeholders and in public hearings.

A final key issue with LLM and agent use is data privacy, and keeping data private would require a modification of our approach. Our approach uses LLMs hosted in the cloud, and data privacy policies of those LLM providers are variable. Fisheries modelling is often using sensitive data and data-use agreements may include the requirement that data not be uploaded to cloud services or leave the country of origin (leigha ref???). Likewise, socio-ecological studies may make use of human interview data that also have data use restrictions. In fact, the most common question we have received from fisheries scientists when we run training workshops on LLMs is "how can I keep the data private?". Open source models like Kimi K2 can be downloaded and hosted on local servers or computers, giving researchers greater control over data privacy. In general, open-source models are not as accurate on evaluations as proprietary models [e.g. @laban2025llms; @zhou2024large], but they may still give researchers a head start on modelling.

## 5. Conclusion

We found AI agents built with contemporary LLMs and generic agent software can reliably and accurately complete common fisheries and ecological modelling tasks. Agentic AI systems represent a promising tool for fisheries science, offering the potential to accelerate the delivery of scientific advice for management and address bottlenecks in quantitatively trained scientists. The analysis provided several insights on which we base some recommendations:

1.  The ecological modelling community needs to develop transparent protocols for sharing AI aided workflows, including sharing of specification sheets for specific tasks. These can then be re-used and potentially assist less specialized users to more accurately and quickly complete modelling tasks.
2.  Future research priorities should include testing agents on real-world datasets with missing data and methodological complexities and expanding evaluations to even more complex modeling problems. For example, agents could be tested to support the full stock assessment process, including fitting multiple empirical models (growth curves, selectivity curves etc...) and performing population dynamics modelling to inform management reference points. Another area of application could be the automation of developing multiple models for management strategy evaluation.

Overall we recommend that AI agents can be used to support fisheries modelling workflows, and are best used by experts who have the knowledge to critically evaluate and correct results.
...
-   Make a repo of the results and share that as well.
